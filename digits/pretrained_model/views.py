...
def validate_archive_keys(info):
    ...
    return next(((False, key) for key in keys if key not in info), (True, 0))
...
def upload_archive():
    ...
    if valid is False:
        return flask.jsonify({"status": f"Missing Key '{key}' in info.json"}), 500
...
def new():
    ...
    framework = "caffe" if 'framework' not in form else form['framework']
